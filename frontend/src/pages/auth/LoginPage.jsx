    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-2xl border border-gf-border bg-gf-surface shadow-sm">
        <div className="p-5 border-b border-gf-border">
          <div className="text-sm font-semibold text-gf-title">Mot de passe oublié</div>
...
export default function LoginPage() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // stocke token + user
      setAuth({ token: res.token, user: res.user }, { persist: remember ? "local" : "session" });
    ...
    <div className="min-h-dvh bg-gf-bg text-gf-text flex items-center justify-center p-6">
      <div className="w-full max-w-md rounded-2xl border border-gf-border bg-gf-surface shadow-sm p-6">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-xs text-gf-subtitle">Accès réservé au personnel de Ginioux Flamary.</div>
        </div>
        ...
        <form className="mt-6 space-y-4" onSubmit={onSubmit}>
          ...
          <div>
            <div className="text-xs text-gf-subtitle mb-1">Mot de passe</div>
            <input
              value={password}
